...
def stream_worker(name, url, storage_path, segment_time, timezone_str="UTC", is_recording=True):
    logger.info(f"Starting worker for stream: {name} (recording={is_recording})")

    # HLS live view output — always active regardless of recording flag
    hls_dir = f"/tmp/hls/{name}"
    os.makedirs(hls_dir, exist_ok=True)
    hls_playlist = os.path.join(hls_dir, "stream.m3u8")
    hls_segment = os.path.join(hls_dir, "seg%d.ts")
    ...
    cmd += [
        # HLS live view output — scale to 1280x720 for browser compatibility
        "-vf", "scale=1280:720",
        "-c:v", "libx264",
        "-preset", "ultrafast",
        "-tune", "zerolatency",
...
def start_stream(stream, storage_path, segment_time):
    ...
    is_recording = stream.get("is_recording", True) != False
    stop_signals[name] = False
    t = threading.Thread(
        target=stream_worker,
        args=(name, url, storage_path, segment_time, stream.get("timezone", "UTC"), is_recording),
        daemon=True
    )
...
def sync_workers():
    ...
    for s in streams:
        ...
        if name in active_threads and active_threads[name].is_alive():
            pass  # Already running — recording flag changes take effect on next restart
        else:
...
# Serve HLS live view segments generated by FFmpeg
@app.route('/hls/<name>/stream.m3u8')
@requires_auth
def hls_playlist(name):
    path = f"/tmp/hls/{name}/stream.m3u8"
    if not os.path.exists(path):
        return "Stream not ready", 404
    return send_from_directory(f"/tmp/hls/{name}", "stream.m3u8", mimetype="application/vnd.apple.mpegurl")

@app.route('/hls/<name>/<path:filename>')
@requires_auth
def hls_segment(name, filename):
    return send_from_directory(f"/tmp/hls/{name}", filename, mimetype="video/mp2t")
